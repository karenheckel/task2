...
public class TopKMapper extends Mapper<Text, Text, Text, Text> {
	...
	public void map(Text key, Text value, Context context)
		...
		long totalDelay = Long.parseLong(fields[0]);
		long numFlights = Long.parseLong(fields[1]);
		double delayRatio = (double) totalDelay / (double) numFlights;
	...
	public void cleanup(Context context) throws IOException, InterruptedException {
		...
		while (pq.size() > 0) {
			...
			context.write(wordAndCount.getWord(), new Text(Double.toString(wordAndCount.getDelayRatio())));
